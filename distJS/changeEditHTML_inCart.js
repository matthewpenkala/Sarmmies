var spanElement_discountFees_SCRIPT = ("https://matthewpenkala.github.io/Sarmmies/distJS/spanElement_discountFees.min.js").toString();
...
setTimeout(() => {
    ...
    setTimeout(() => {
        $.getScript(spanElement_discountFees_SCRIPT)
    }, 99);
}, 9999);
$("#snipcart").find("*").addBack().add("a.cart-button[href]").not("head").add(".snipcart-checkout, .w-commerce-commercecartopenlink").not("script, style").add('input[class*="add"][data-node-type*=commerce-add-to-cart-button]').click(function () {
    ...
    const reorderShippingRatesJS = ("https://matthewpenkala.github.io/Sarmmies/distJS/reorderShippingRates.js").toString();
    $("button[class*='snipcart']").contents("*").addBack().not("ul, li").click(function () {
        setTimeout(() => {
            $.getScript(reorderShippingRatesJS)
        }, 99), setTimeout(() => {
            $.getScript(reorderShippingRatesJS)
        }, 499), setTimeout(() => {
            $.getScript(reorderShippingRatesJS)
